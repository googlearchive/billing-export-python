...
function BillingExportController($scope,$http){
    ...
    function resourceSelected(){
        ...
        for(var col=0;col < data.getNumberOfColumns();col++){
            ...
            var product = label.substr(0,label.indexOf('/'));
            if(($scope.product == 'ALL') || (product == $scope.product)){
    ...
    function handleQueryResponse(response){
        ...
        function drawTreeChart(rowIndex){
            ...
            for(var col=1;col < data.getNumberOfColumns();col++){
                var label = data.getColumnLabel(col);
                var sku = label.substr(label.indexOf('/')+1,label.length);
                var root = label.substr(0,label.indexOf('/'));
                if(typeof tree[root] == 'undefined'){
                    tree[root] = 0;
                }
                var cell = data.getValue(rowIndex,col);
                tree[root] += cell;
                treeDataTable.rows.push({c:[{v:sku},{v:root},{v:cell}]});
            }
            ...
            $scope.products = ['ALL'];
            for (root  in tree){
                treeDataTable.rows.push({c:[{v:root},{v:'Cloud'},{v:tree[root]}]});
                $scope.products.push(root);
            }
            $scope.$apply();
        ...
        function drawLineChart(){
            // taken from http://jsfiddle.net/rzbc7/
            // allow showing/hiding of series data by clicking on the series.
            var columns = [];
            var series = {};
